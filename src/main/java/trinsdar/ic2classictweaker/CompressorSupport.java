...
public class CompressorSupport {
    ...
    public static void addRecipe(IItemStack output, IIngredient input, @Optional float exp) {
        CraftTweakerActions.apply(new BasicRecipeAddManager(ClassicRecipes.compressor, IC2RecipeInputs.of(input),null, exp, CraftTweakerMC.getItemStack(output)));
    }

    @ZenMethod
    public static void addLateRecipe(IItemStack output, IIngredient input, @Optional float exp) {
        CraftTweakerActions.apply(new BasicRecipeAddManager(ClassicRecipes.compressor, IC2RecipeInputs.of(input),null, exp, CraftTweakerMC.getItemStack(output)).setLate(true));
    }
